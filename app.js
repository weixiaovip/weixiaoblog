...
app.use(bodyParser.json()); //请求体是json时
...
app.use(flash());
//把session中的user对象先传入要渲染的页面模板中
app.use(function (req, res, next) {
    // res.locals是真正用来渲染模板的对象
    res.locals.user = req.session.user;
    res.locals.success = req.flash('success').toString(); //一个类型的flash数据可以有多条，因此转为字符串
